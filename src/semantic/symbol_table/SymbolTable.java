 ...
 * Its interfaces are like map reg structure, but with extra scope mechanism.
 *
 * Without enough sophisticated reg structure, we adopt imperative front.Symbol table.
...
class Binder<T> {
  T value;
  String prev_stack_key;
  Binder<T> overridden_binder;
  Binder(T v, String pre_k, Binder<T> pre_b) { value = v; prev_stack_key = pre_k; overridden_binder = pre_b; }
...
public class SymbolTable<T> {
  ...
  private java.util.Hashtable<String, Binder<T>> table = new java.util.Hashtable<>();
  private String top_stack_key = null; // latest inserted key, located at top of stack.
...
//    System.out.println("end scope\n");
   ...
   * */
  public boolean ExistCurScope(String key) {
    Binder<T> now_binder;
    String cur_key = top_stack_key;
    while (cur_key != null) {
